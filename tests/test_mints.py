...
class CollectorTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_targeted_native_workers_do_not_start_full_chain_indexer(self):
        ...
        self.assertEqual(sorted(started),["coingecko:status","flow:status","holder_history:status","mints:history","mints:status","network_weight:status","powder:status","provenance:balances","provenance:burns","provenance:incoming","provenance:status","public_holders:GNK:status","snapshot:status"])
        self.assertEqual({t.get_name() for t in self.idx.tasks},{"coingecko_circulating","gonka_auto_snapshot","gonka_network_weight","wgnk_mints_live","wgnk_mints_history","wgnk_market_flow","stable_powder","gonka_bridge_links","gonka_burn_links","gonka_targeted_incoming","gonka_targeted_balances","gonka_holder_census","gonka_holder_history"})
        await self.idx.stop()

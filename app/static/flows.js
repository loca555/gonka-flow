...
function renderLiveDepth(data){
  ...
  const rows=Object.entries(live.bands).sort((a,b)=>a[0]-b[0]).map(([lvl,b])=>
   '<span>±'+lvl+'% <b class="pnl-pos">↑ '+money(+b.up_usdt_raw/1e6)+'</b> <b class="pnl-neg">↓ '+money(+b.down_usdt_raw/1e6)+'</b></span>').join(" ");
  box.innerHTML='Точно сейчас, по тикам пула 30 б.п. (цена $'+(+live.price).toFixed(4)+'): '+rows;
  box.hidden=false;
 }

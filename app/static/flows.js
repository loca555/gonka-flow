...
(()=>{
 ...
 const flow={hours:0,minimum:10000,q:"",offset:0,side:"all",sort:"time_desc",minterSort:"sold_desc",data:null,loading:false,foreground:false,request:0};
 const pages=new Map();
 ...
 function mount(){
  ...
  '<form id="sales-volume-filter" class="volume-filter" aria-label="Фильтры сделок"><label for="sales-side">Сделки <select id="sales-side"><option value="all">Покупки и продажи</option><option value="sell">Только продажи</option><option value="buy">Только покупки</option></select></label><label for="sales-minimum">Объём от <input id="sales-minimum" type="number" min="0" max="1000000000000" step="1" value="10000" inputmode="numeric" title="Минимальный объём в целых WGNK; 0 — любой объём"> <span>WGNK</span></label><button type="submit">Применить</button><button type="button" data-volume-reset disabled>Сбросить</button></form>' +
  '<div class="table-container"><table id="trades-table"><thead><tr><th data-sort="time">Дата и время</th><th data-sort="kind" data-default="asc">Сделка</th><th data-sort="actor" data-default="asc">Адрес / инициатор</th><th data-sort="pnl" data-default="desc" class="numeric" title="PNL адреса: меньший из объёмов (куплено/продано) × (цена продажи − цена покупки), USDT. Незакрытая часть не учитывается; скрыто при результате в пределах ±100 USDT">PNL · USDT</th><th data-sort="amount" class="numeric">Объём WGNK</th><th data-sort="quote" class="numeric">Сумма USDT</th><th data-sort="price" class="numeric">Цена за 1 WGNK, USDT</th><th data-sort="pool" data-default="asc">Пул</th><th data-sort="tx" data-default="asc">Транзакция</th></tr></thead><tbody id="sales-rows"></tbody></table></div>'+
 ...
 function tradePnlCell(e,data){
  const raw=e.actor&&data&&data.pnl?data.pnl[e.actor]:null;
  if(raw==null)return '<td class="numeric" title="Нет PNL: одна сторона торговли или результат в пределах ±100 USDT"></td>';
  const n=BigInt(raw),neg=n<0n,abs=neg?-n:n,value=amount((abs/10n**6n).toString());
  return '<td class="numeric" title="PNL адреса: меньший из объёмов (куплено/продано) × (цена продажи − цена покупки), USDT. Незакрытая часть не учитывается; скрыто при результате в пределах ±100 USDT"><span class="'+(neg?'pnl-neg':'pnl-pos')+'">'+(neg?'−':'+')+' '+esc(value)+'</span></td>';
 }
 ...
 function renderTradePage(data){
  ...
  setLiveHTML(ui("sales-rows"),data.trades.length?data.trades.map(e=>'<tr><td>'+date(e.ts)+'<small>'+clock(e.ts)+'</small></td><td><span class="trade-badge '+e.kind+'">'+(e.kind==="buy"?"Покупка":"Продажа")+'</span></td><td>'+
   (e.attribution==="initiator_net"||e.attribution==="tx_net"?addr(e.actor):'<span class="mono" title="'+esc(e.actor)+'">'+esc(e.actor?short(e.actor,10):"Не установлен")+'</span>')+
   '<small>'+(e.attribution==="initiator_net"?(e.kind==="buy"?"Приток WGNK подтверждён":"Отток WGNK подтверждён"):e.attribution==="tx_net"?(e.kind==="buy"?"Получатель WGNK подтверждён":"Плательщик WGNK подтверждён"):e.attribution==="initiator_only"?"Только инициатор исполнения":"Участник не установлен")+'</small></td>'+tradePnlCell(e,data)+'<td class="numeric">'+esc(fmt(e.amount))+'</td><td class="numeric">'+esc(amount(e.quote))+'</td><td class="numeric price-value">'+esc(price(e.price))+'</td><td><small>Uniswap V3</small>'+count((data.pools.find(p=>p.address===e.pool)?.fee||0)/100)+' б.п.</td><td><a class="tx-link" href="'+txUrl(e.tx_hash)+'" target="_blank" rel="noopener noreferrer">'+esc(short(e.tx_hash))+' ↗</a><small>Swap #'+e.idx+'</small></td></tr>').join(""):
   '<tr><td colspan="9" class="empty">Сделок по этим фильтрам в отслеживаемых пулах не найдено.</td></tr>');
 }
 ...
 mount();refresh();
 const LIQ_LEVELS_KEY='gonka-liquidity-levels';
 ...
 function renderLiquidityChart(data){
  ...
  renderLiveDepth(data);
}
 function renderLiveDepth(data){
  const box=ui("liq-live-depth");if(!box)return;
  const live=data.depth_live;
  if(!live||!live.bands){box.hidden=true;return;}
  const money=v=>"$"+(v>=1e6?(v/1e6).toLocaleString("ru-RU",{maximumFractionDigits:2})+" млн":v>=1e3?(v/1e3).toLocaleString("ru-RU",{maximumFractionDigits:1})+" тыс":Math.round(v).toLocaleString("ru-RU"));
  const rows=Object.entries(live.bands).filter(([,b])=>b&&b.up_usdt_raw&&b.down_usdt_raw).sort((a,b)=>a[0]-b[0]).map(([lvl,b])=>
   '<span title="Цена после сделки: ↑ $'+(+b.up_final_price).toFixed(4)+' · ↓ $'+(+b.down_final_price).toFixed(4)+'">±'+lvl+'% <b class="pnl-pos">↑ '+money(+b.up_usdt_raw/1e6)+'</b> <b class="pnl-neg">↓ '+money(+b.down_usdt_raw/1e6)+'</b></span>').join(" ");
  if(!rows){box.hidden=true;return;}
  box.innerHTML='Точно сейчас, по тикам пула 30 б.п. · цена $'+(+live.price).toFixed(4)+' · средний курс исполнения (комиссия 0,3% учтена): '+rows;
  box.hidden=false;
 }

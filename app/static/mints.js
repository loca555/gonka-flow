...
function renderChart(d){
 ...
 const chart=el("mint-chart"),options={points:daily.map(x=>({date:x.date,raw:x.amount_raw,events:x.events,price_raw:x.price_raw,weight:x.weight,gpus:x.gpus})),
 ...
 let window_note=(d.coverage.complete?"Финальная история":"Есть пропуски истории")+" · текущий день может быть неполным · оранжевая линия — среднее за 7 дней · голубая — цена дня (правая ось) · фиолетовая — вес сети GPU-майнеров (внешняя ось)";

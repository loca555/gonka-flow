...
(async()=>{
 ...
 for(const step of ['5','10'])assert.equal(data.price_days[step].bands.reduce((sum,b)=>sum+b.days,0),data.price_days[step].total_days);
 ...
 try{
  ...
  const fixture={...data,price_distribution:{'5':bands,'10':bands},price_days:{'5':dayFixture,'10':dayFixture}};

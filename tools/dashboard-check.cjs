...
(async()=>{
 ...
 try{
  ...
  assert.deepEqual([...mints.workers].sort(),["gonka_bridge_links","gonka_burn_links","gonka_targeted_balances","gonka_targeted_incoming","wgnk_market_flow","wgnk_mints_history","wgnk_mints_live"]);
  ...
  assert.equal(await page.evaluate(()=>price("0.123456789123")),"0.123457");
  assert.equal(await page.evaluate(()=>price("0.000000000001")),"< 0.000001");
  assert.equal(await page.evaluate(()=>price("0")),"0");
  assert.equal(await page.evaluate(()=>price(null)),"—");
  assert.equal(await page.evaluate(()=>price("12345678901234567890.123456").replace(/\s/g," ")),"12 345 678 901 234 567 890.123456");
  assert.match(await page.locator('#minter-table th[data-sort="price"]').innerText(),/Средняя цена за 1 WGNK, USDT/);
  ...
  assert.match(await page.locator("#mint-detail").innerText(),/статус зачисления GNK — в истории адреса/);
  ...
  assert.match(await page.locator("#sales-chart .chart-tooltip").innerText(),/USDT за 1 WGNK/);
  assert.equal(await page.locator("#sales-chart .chart-unit.price-axis").textContent(),"USDT / 1 WGNK");
  assert.equal(await page.locator("#flow-price").innerText(),await page.evaluate(v=>price(v),chartData.summary.average_price));
  ...
  const chartFixtures=await page.evaluate(()=>{
   ...
   try{
    ...
    const unitPrice=host.querySelector(".chart-tooltip").textContent.includes("Цена: 0.2");
    ...
    return {stack,ratio,gaps,gapTooltip,buyOnly,unitPrice,empty:!host.querySelector("svg")};
   }finally{host.remove();}
  });
  assert.equal(chartFixtures.stack,true);assert.equal(chartFixtures.ratio,3);assert.equal(chartFixtures.unitPrice,true);
  ...
  assert.doesNotMatch((await page.locator("td.numeric:not(.price-value)").allTextContents()).join(" "),/\d[.,]\d/);
  assert.doesNotMatch((await page.locator("td.price-value").allTextContents()).join(" "),/\d,\d/);
  assert.doesNotMatch(await page.locator("#flow-price-label").innerText(),/000/);

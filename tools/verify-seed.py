...
with tempfile.TemporaryDirectory(prefix='gonka-seed-check-') as tmp:
    ...
    try:
        ...
        assert all(key in {'mints:deployment','mints:status','mints:next','mints:bootstrapped','flow:snapshot','flow:status','public_holders:GNK','holder_history:version','coingecko:wgnk','network_weight:history','inference_tokens:history','flow:depth_history','lp_ticks:head'} for key in keys),keys

...
with tempfile.TemporaryDirectory(prefix='gonka-seed-check-') as tmp:
    ...
    try:
        ...
        assert all(key in {'mints:deployment','mints:status','mints:next','mints:bootstrapped','flow:snapshot','flow:status','public_holders:GNK','holder_history:version'} for key in keys),keys

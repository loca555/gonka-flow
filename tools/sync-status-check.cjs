...
(async()=>{
 ...
 try{
  ...
  const fixture=name=>{
   ...
   d.now=now;
   if(name==='mints'){
    d.indexer_enabled=!scenario.paused;d.coverage.live.checked_at=now;d.coverage.live.latest_ts=now;d.coverage.live.latest_height=market.coverage.head+(scenario.liveTail||0);
  ...
  for(const [name,value,expected] of [
   ...
   ['Live head does not hide index lag',{liveTail:84,marketLag:100},'syncing'],
   ['Live head does not hide RPC errors',{liveTail:84,rpcError:true},'error'],
   ['Live head does not hide stale workers',{liveTail:84,staleMarket:true},'stale'],
   ['Recovery',{},'live']
  ]){
   ...
   if(value.liveTail)assert(!text.includes('финализаци'));

...
(async()=>{
 ...
 try{
  ...
  const fixture=name=>{
   ...
   d.now=now;
   if(name==='mints'){
    d.indexer_enabled=!scenario.paused;d.coverage.live.checked_at=now;d.coverage.live.latest_ts=now;
    ...
    if(scenario.newHead){d.coverage.head+=scenario.newHead;d.coverage.total+=scenario.newHead;d.coverage.covered+=scenario.newHead;}
    if(scenario.staleMint)d.coverage.live.checked_at-=100;
   }else if(name==='market'){
    d.status={...d.status,checked_at:now,error:null,ok:true};d.snapshot.checked_at=now;
  ...
  assert((await page.locator('#snapshot-detail').textContent()).includes('ждём новые блоки'));
